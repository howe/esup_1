...
{
    $sql = "SELECT g.rec_id, g.goods_id, (select yxm from " .$GLOBALS['ecs']->table('goods')." where goods_id = g.goods_id) yxm, ". 
			"(select yxqm from " .$GLOBALS['ecs']->table('goods')." where goods_id = g.goods_id) yxqm, ". 
			"(select goods_type from " .$GLOBALS['ecs']->table('goods')." where goods_id = g.goods_id) goods_type, ".
			"(select yxfm from " .$GLOBALS['ecs']->table('goods')." where goods_id = g.goods_id) yxfm, ".
			"(select danwei from " .$GLOBALS['ecs']->table('goods')." where goods_id = g.goods_id) danwei, ".
			"(select danjia from " .$GLOBALS['ecs']->table('goods')." where goods_id = g.goods_id) danjia, ".
			"(select yxsl from " .$GLOBALS['ecs']->table('goods')." where goods_id = g.goods_id) yxsl, ".
			"(select zhenying from " .$GLOBALS['ecs']->table('goods')." where goods_id = g.goods_id) zhenying, ".
			"g.goods_name, g.goods_sn, g.market_price, g.goods_number, " .
            "g.goods_price, g.goods_attr, g.is_real, g.parent_id, g.is_gift, " .
            "g.goods_price * g.goods_number AS subtotal, g.extension_code " .
            "FROM " . $GLOBALS['ecs']->table('order_goods') .
            " as g WHERE order_id = '$order_id'";
