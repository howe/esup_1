...
/* david PHPThumb */
require_once(ROOT_PATH . 'includes/PHPThumb/ThumbLib.inc.php');
class cls_image
{
    var $error_no    = 0;
    var $error_msg   = '';
	...
	/* david 用PHPThumb创建缩略图 */
	$thumb_options = array('resizeUp' => true, 'jpegQuality' => 94);
	$thumb = PhpThumbFactory::create($img, $thumb_options);//
	$thumb->adaptiveResize($thumb_width, $thumb_height);
	$thumb->save($dir.$filename, $img_format);
    ...
    function add_watermark($filename, $target_file='', $watermark='', $watermark_place='', $watermark_alpha = 0.65)
    {
        ...
        switch ($source_info[2] )
        {
            ...
            case 'image/jpeg':
            case 2:
                imagejpeg($source_handle, $target, 94);//david 相册原图加水印后的图片质量
                break;
