...
function virtual_card_shipping ($goods, $order_sn, &$msg, $process = 'other')
{
    ...
    if ($num < $goods['num'])
    {
     ...
     foreach ($arr as $virtual_card)
     {
		...
		$card_info['card_url'] = $virtual_card['card_url'];
        $cards[] = $card_info;
     }
...
function virtual_card_result($order_sn, $goods)
{
    ...
    /* 获取已经发送的卡片数据 */
    $sql = "SELECT card_sn, card_password, card_url, end_date, crc32 FROM ".$GLOBALS['ecs']->table('virtual_card')." WHERE goods_id= '$goods[goods_id]' AND order_sn = '$order_sn' ";
    ...
    while ($row = $GLOBALS['db']->FetchRow($res))
    {
        ...
        $cards[] = array('card_sn'=>$row['card_sn'], 'card_url'=>$row['card_url'], 'card_password'=>$row['card_password'], 'end_date'=>date($GLOBALS['_CFG']['date_format'], $row['end_date']));
    }
