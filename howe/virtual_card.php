...
elseif ($_REQUEST['act'] == 'edit_replenish')
{
    ...
    admin_priv('virualcard');
    /* 获取卡片信息 */
    $sql = "SELECT T1.card_id, T1.goods_id, T2.goods_name, T1.card_url, T1.card_sn, T1.card_password, T1.end_date, T1.crc32 FROM ".
...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    if (empty($_POST['old_card_sn']))
    {
        ...
        $add_date = gmtime();
        $sql = "INSERT INTO ".$ecs->table('virtual_card')." (goods_id, card_url, card_sn, card_password, end_date, add_date, crc32) ".
               "VALUES ('$_POST[goods_id]','$_POST[card_url]', '$coded_card_sn', '$coded_card_password', '$end_date', '$add_date', '" . crc32(AUTH_KEY) . "')";
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'batch_insert')
{
    ...
    foreach ($_POST['checked'] as $key)
    {
		...
		$rec['card_url']  = $_POST['card_url'][$key];
...
function get_replenish_list()
{
    ...
    /* 查询 */
    $sql = "SELECT card_id, goods_id, card_sn, card_url, card_password, end_date, is_saled, order_sn, crc32 ".
