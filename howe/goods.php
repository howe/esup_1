...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	$danwei = empty($_POST['danwei']) ? '' : $_POST['danwei'];
	$danjia = empty($_POST['danjia']) ? $_POST['shop_price'] : $_POST['danjia'];
	$yxsl = empty($_POST['yxsl']) ? 1 : $_POST['yxsl'];
	$zhenying = empty($_POST['zhenying']) ? '' : $_POST['zhenying'];
	$ghsp_id = empty($_POST['ghsp_id']) ? '' : $_POST['ghsp_id'];
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
					...
					"danwei, danjia, yxsl, zhenying, ghsp_id, yxfm, yxqm, yxm)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$danwei', '$danjia', '$yxsl', ".
					" '$zhenying', '$ghsp_id', (SELECT cat_name FROM " . $ecs->table('category') . " WHERE cat_id = '$catgory_id'), ".
					" (SELECT cat_name FROM " . $ecs->table('category') . " WHERE cat_id = (SELECT parent_id FROM " . $ecs->table('category') . " WHERE cat_id = '$catgory_id')), ".
					" (SELECT cat_name FROM " . $ecs->table('category') . " WHERE cat_id = (SELECT parent_id FROM " . $ecs->table('category') . " WHERE cat_id = (SELECT parent_id FROM " . $ecs->table('category') . " WHERE cat_id = '$catgory_id'))))";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$danwei', '$danjia', '$yxsl', ".
					" '$zhenying', '$ghsp_id', (SELECT cat_name FROM " . $ecs->table('category') . " WHERE cat_id = '$catgory_id'), ".
					" (SELECT cat_name FROM " . $ecs->table('category') . " WHERE cat_id = (SELECT parent_id FROM " . $ecs->table('category') . " WHERE cat_id = '$catgory_id')), ".
					" (SELECT cat_name FROM " . $ecs->table('category') . " WHERE cat_id = (SELECT parent_id FROM " . $ecs->table('category') . " WHERE cat_id = (SELECT parent_id FROM " . $ecs->table('category') . " WHERE cat_id = '$catgory_id'))))";
        }
    ...
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
				...
				"yxsl = '$yxsl', " .
				"zhenying = '$zhenying', " .				
				"yxm = (SELECT cat_name FROM " . $ecs->table('category') . " WHERE cat_id = (SELECT parent_id FROM " . $ecs->table('category') . " WHERE cat_id = (SELECT parent_id FROM " . $ecs->table('category') . " WHERE cat_id = '$catgory_id'))), " .
				"yxqm = (SELECT cat_name FROM " . $ecs->table('category') . " WHERE cat_id = (SELECT parent_id FROM " . $ecs->table('category') . " WHERE cat_id = '$catgory_id')), " .
				"yxfm = (SELECT cat_name FROM " . $ecs->table('category') . " WHERE cat_id = '$catgory_id'), " .
                "ghsp_id = '$ghsp_id', " .				
