...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update','danjia'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if ($cat['grade'] > 1)
    {
        ...
        */
        ...
        $sql = "SELECT min(g.shop_price) AS min, max(g.shop_price) as max ".
               ...
               " WHERE ($children OR " . get_extension_goods($children) . ') AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.goods_number > 0 ';
               //获得当前分类下商品价格的最大值、最小值
    ...
    $sql = "SELECT b.brand_id, b.brand_name, COUNT(*) AS goods_num ".
            ...
            " AND g.is_on_sale = 1 AND g.goods_number > 0 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        {
            $sql = "SELECT a.attr_name FROM " . $ecs->table('attribute') . " AS a, " . $ecs->table('goods_attr') . " AS ga, " . $ecs->table('goods') . " AS g WHERE ($children OR " . get_extension_goods($children) . ") AND a.attr_id = ga.attr_id AND g.goods_id = ga.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.goods_number > 0 AND g.is_alone_sale = 1 AND a.attr_id='$value'";
            if($temp_name = $db->getOne($sql))
            {
                ...
                $sql = "SELECT a.attr_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value FROM " . $ecs->table('goods_attr') . " AS a, " . $ecs->table('goods') .
                       " AS g" .
                       " WHERE ($children OR " . get_extension_goods($children) . ') AND g.goods_id = a.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.goods_number > 0 AND g.is_alone_sale = 1 '.
                       " AND a.attr_id='$value' ".
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $where = "g.is_on_sale = 1 AND g.goods_number > 0 AND g.is_alone_sale = 1 AND ".
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.yxm, g.yxqm, g.goods_number, g.zhenying, g.yxfm, g.danwei, g.danjia, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['goods_id']]['yxm']             = $row['yxm'];
		$arr[$row['goods_id']]['yxqm']             = $row['yxqm'];
		$arr[$row['goods_id']]['yxfm']             = $row['yxfm'];
		$arr[$row['goods_id']]['zhenying']             = $row['zhenying'];
		$arr[$row['goods_id']]['goods_number']             = $row['goods_number'];
		$arr[$row['goods_id']]['danjia']             = $row['danjia'];
		$arr[$row['goods_id']]['danwei']             = $row['danwei'];
...
{
    $where  = "g.is_on_sale = 1 AND g.goods_number > 0 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ')';
